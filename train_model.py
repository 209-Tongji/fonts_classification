...
def mygenerator(generator):
    ...
    while True:
        ...
        yield data_x, data_y
# training model
def model_train(model, loadweights, isCenterloss, lambda_center=0.01):#houlianggecanshu quexing
    lr = LearningRateScheduler(lrschedule)
    mdcheck = ModelCheckpoint(WEIGHTS_PATH, monitor='val_accuracy', save_best_only=True)
...
def get_label_predict_top_k(image, model, top_k):
        ...
        """
        # array 2 list
        predict_properly = model.predict(image)
        predict_list = list(predict_properly[0])
...
def save_csv(test_image_path, predict_label):
        ...
        save_arr.to_csv('submit_test.csv', decimal=',', encoding='ansi', index=False, index_label=False)
        print('submit_test.csv have been write, locate is :', os.getcwd())

    # main function
if __name__ == "__main__":
        train_path = code_path + 'train/'
        val_path = code_path + 'val/'
        ...
        simple_model.load_weights(WEIGHTS_PATH)
        model = simple_model
        predict_label = test_image_predict_top_k(model, code_path + test_image_path, train_path, 1)
